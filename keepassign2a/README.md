...
4. Run the backend `npm run jsonserver`  which shall run on port:3000 , this command need globle json-server-auth cli 
5. Run the frontend `npm run start` which shall run on port:4200  

## Know your server  

On running `npm run jsonserver`, following apis would be available for your use -  
1. To authenticate user - POST - http://localhost:3000/login - expecting data - { email, password }  
2. To check if user is authenticated - POST - http://localhost:3000/660/users - expecting header - { 'Authorization', `Bearer ${accessToken}` }  
3. To get notes - GET - http://localhost:3000/660/notes - expecting header - { 'Authorization', `Bearer ${accessToken}` }  
4. To add a note - POST - http://localhost:3000/660/notes - expecting header - { 'Authorization', `Bearer ${accessToken}` } and data - { note }  
...
7. `DashboardComponent` which displays the `HeaderComponent` and one other component based on the requested route to contain the same Expansion Panel and the Notes Collection from `AppComponent` implemented in `Keep Level 1` , 
...
11. `RouterService` to navigate user to available routes -  
	...
	11.3 `routeToLogOut()` to navigate to logOut route   
...
`npm run test
`
14. Ensure e2e test cases pass -  (options)
`npm run jsonserver` (backend shall be running before executing e2e test cases)  
`npm run e2e`
